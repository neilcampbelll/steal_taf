## Top-level workflow wrappers for a stockassessment.org -> TAF assessment
##
## Usage:
##   source("run.R")
##
...
##   steal_taf("user3", "WBCod_2027_final")  # download into boot/
##   run_taf()                                # then run the analysis
##
##   # If boot/data is already populated, just rerun the analysis
##   run_taf()
##   run_taf(clean = TRUE)                              # wipe + rerun
##   run_taf(steps = c("output.R", "report.R"))         # only later stages
...
##   run_name : assessment run name, e.g. "WBCod_2027_final"
...
run_taf <- function(clean = FALSE,
                    boot  = FALSE,
                    steps = c("data.R", "model.R", "output.R", "report.R")) {
